...
type FuzzyTextProps = {
  ...
  burstRiseDuration?: number
  burstFallDuration?: number
}
...
const FuzzyText = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      let lastBurstSignal = burstSignalRef.current
      ...
      if (burstSignalRef.current > 0) {
        startBurst()
      }

      const getBurstIntensity = (timestamp: number) => {
        const elapsed = timestamp - burstStartTime
        const totalDuration = burstRiseDuration + burstFallDuration
        ...
        if (elapsed <= burstRiseDuration) {
          const progress = elapsed / burstRiseDuration
          return baseIntensity + (burstIntensity - baseIntensity) * progress
        }

        const fallProgress = (elapsed - burstRiseDuration) / burstFallDuration
        return burstIntensity - (burstIntensity - baseIntensity) * fallProgress
      }
      ...
      const run = (timestamp: number) => {
        ...
        if (burstSignalRef.current !== lastBurstSignal) {
          lastBurstSignal = burstSignalRef.current
          startBurst()
        }
        ...
        if (burstActive) {
          targetIntensity = getBurstIntensity(timestamp)
        } else if (isClicking) {
          targetIntensity = 1
        } else if (isGlitching) {
          targetIntensity = 1
        ...
        if (transitionDuration > 0 && !burstActive) {
